	...
	public Board() {
		...
		for (int i = 0; i < _board.length; i++) {
			for (int j = 0; j < _board.length; j++) {
				...
				/*
				 * if (flag) { if (count == 4) { _board[i][j] = new Square(Soldier.BLACK); count
				 * = 0; } else { _board[i][j] = new Square(Soldier.WHITE); count++; } } else {
				 * if (count == 4) { _board[i][j] = new Square(Soldier.WHITE); count = 0; } else
				 * { _board[i][j] = new Square(Soldier.BLACK); count++; } } } flag = !flag;
				 * 
				 * _board[14][14] = new Square(Soldier.EMPTY);
				 */
			}
		}
	}
